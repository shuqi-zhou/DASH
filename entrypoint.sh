...
case "${CMD}" in
    demo)
        ...
        python pipeline/step3_HMM_assign.py --output_dir output --batch
        ;;
    all)
    ...
    step3)
        shift
        echo "[DASH] Running pipeline/step3_HMM_assign.py $*"
        python pipeline/step3_HMM_assign.py --output_dir output "$@"
        ;;
    bash|sh)
        shift
        exec /bin/bash "$@"
