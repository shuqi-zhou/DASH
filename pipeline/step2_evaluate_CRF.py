...
# Modified 2026-05-31 for ver1/ver2 CRF comparison reproducibility.
# Keep inference full precision, preserve trace order, and write comparison outputs separately.
...
class IndexedSequenceDataset(SequenceDataset):
    """Return original trace index so batch length-sorting does not change saved order."""

    def __getitem__(self, index):
        sequence, label, weight = super().__getitem__(index)
        return sequence, label, weight, index


def collate_fn_with_index(batch):
    indexed_batch = [(sequence, label, weight, idx) for sequence, label, weight, idx in batch]
    indexed_batch.sort(key=lambda x: len(x[0]), reverse=True)
    original_indices = torch.tensor([idx for _, _, _, idx in indexed_batch], dtype=torch.long)
    sorted_samples = [(sequence, label, weight) for sequence, label, weight, _ in indexed_batch]
    sequences, labels, weights = collate_fn(sorted_samples)
    return sequences, labels, weights, original_indices
...
def print_and_save_stats(pred_seg, class_counts, total_tokens, data_file, stat_path):
    ...
    lines += [
        "-" * 60,
        "Trace-level segment presence (non-exclusive; one trace may contain both types):",
        (f"Traces containing seg-type1 (start=1): {traces_with_seg1}/{n}"
         f"  ({100*traces_with_seg1/n:.1f}%)") if n else "No traces",
        (f"Traces containing seg-type2 (start=4): {traces_with_seg4}/{n}"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default=str(PROJECT_ROOT / 'step2'),
                        help='Directory for prediction/stat/config outputs')
    ...
    # Modified 2026-05-31: disable AMP so CRF/Viterbi decisions match ver1 full precision.
    use_amp = False
    print("AMP disabled for full-precision inference")

    # ── Data files ────────────────────────────────────────────────────────────
    test_data_path = _cfg.get('test_data_path', '')
    if test_data_path and not os.path.exists(test_data_path):
        fallback_test_data_path = LOCAL_DATA_ROOT / 'step0_preprocess' / 'test'
        if fallback_test_data_path.exists():
            print(f"Configured test_data_path not found: {test_data_path}")
            print(f"Using local test_data_path: {fallback_test_data_path}")
            test_data_path = str(fallback_test_data_path)
    ...
    # ── Model ─────────────────────────────────────────────────────────────────
    ckpt_candidates = [
        os.path.join(root, 'output', 'ckpt_CRF', ckpt, load_ckpt),
        os.path.join(root, 'ckpt_CRF', ckpt, load_ckpt),
        str(LOCAL_DATA_ROOT / 'ckpt_CRF' / ckpt / load_ckpt),
    ]
    ...
    # ── Output directory ──────────────────────────────────────────────────────
    # Modified 2026-05-31: save comparison outputs outside the normal eval directory.
    eval_output_dir = os.path.abspath(args.output_dir)
    ...
    for file_idx, data_file in enumerate(data_files):
        ...
        val_loader = DataLoader(
            IndexedSequenceDataset(data_val),
            collate_fn=collate_fn_with_index,
        ...
        model.eval()
        with torch.no_grad():
            for sequences, labels, weights, original_indices in tqdm(val_loader, desc="Evaluating"):
                ...
                for seq_i, original_idx in enumerate(original_indices.tolist()):
                    pred_seg[original_idx] = batch_pred_infos[seq_i]
        ...
        assert all(item is not None for item in pred_seg)
