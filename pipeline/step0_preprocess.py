...
def normalize_sep(input_data):
    ...
    for raw_trace, label, meta in input_data:
        ...
        # FRET efficiency  (clip to non-negative first)
        donor    = np.maximum(trace_norm[:, 0], 0)
        acceptor = np.maximum(trace_norm[:, 1], 0)
        total    = donor + acceptor
        with np.errstate(divide="ignore", invalid="ignore"):
            fret_eff = np.where(total > 1e-10, acceptor / total, 0.0)
            fret_eff = np.nan_to_num(fret_eff, nan=0.0, posinf=0.0, neginf=0.0)
