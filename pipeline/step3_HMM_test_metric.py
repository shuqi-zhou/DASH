...
NSTATES_VALUES          = [2, 3, 4, 5, 6, 7] # number of FRET states to try
GLOBAL_THRESHOLD_VALUES = [0.3, 0.2, 0.1]
LOCAL_THRESHOLD_VALUES  = [0.15, 0.12]


# ---------------------------------------------------------------------------
#  Metrics  (GPU-accelerated where possible)
# ---------------------------------------------------------------------------

def viterbi_path_consistency(hidden_states, length_info):
    """Fraction of frames without a state change (higher = more consistent)."""
    # Use GPU tensor ops
    hs = torch.tensor(hidden_states, dtype=torch.int32, device=DEVICE)
    start = 0
    scores = []
    for length in length_info:
        ...
        trace = hs[start:end]
        if length > 1:
            transitions = (trace[1:] != trace[:-1]).sum().item()
            scores.append(1.0 - transitions / (length - 1))
        else:
            scores.append(1.0)
        start = end
...
def calculate_merged_bic(fret_values, merged_states):
    ...
    # Move to GPU for per-state mean/std
    fret_t  = torch.tensor(fret_values, dtype=torch.float32, device=DEVICE)
    states_t = torch.tensor(merged_states, dtype=torch.int64, device=DEVICE)

    log_likelihood = 0.0
    for state in unique_states:
        mask       = (states_t == int(state))
        state_data = fret_t[mask]
        if state_data.numel() > 1:
            mu  = state_data.mean().item()
            std = state_data.std().item()
            if std == 0:
                std = 1e-6
    ...
    return max(0.0, 1.0 - (bic / 1_000_000))
...
def state_separation_clarity(fret_values, hidden_states):
    ...
    fret_t  = torch.tensor(fret_values, dtype=torch.float32, device=DEVICE)
    states_t = torch.tensor(hidden_states, dtype=torch.int64, device=DEVICE)
    ...
    for s in unique_states:
        ...
        if sd.numel() == 0:
            ...
            state_stds.append(1e-6)
        else:
            state_means.append(sd.mean().item())
            state_stds.append(sd.std().item() if sd.numel() > 1 else 1e-6)
...
def dwell_time_score(hidden_states, length_info):
    ...
    if len(dwell_times) < 5:
        return 0.0
    dwell_times = np.array(dwell_times, dtype=np.float32)
...
def physical_realism_score(hidden_states, fret_values):
    ...
    for s in unique_states:
        mask = (states_t == int(s))
        sd   = fret_t[mask]
        state_means.append(sd.mean().item() if sd.numel() > 0 else 0.0)
...
def transition_matrix_entropy(hmm_model):
    trans_mat = hmm_model.transmat_
    tm = torch.tensor(trans_mat, dtype=torch.float32, device=DEVICE) + 1e-10
    tm = tm / tm.sum(dim=1, keepdim=True)
    entropy     = -(tm * torch.log2(tm)).sum(dim=1)
    max_entropy = torch.log2(torch.tensor(float(tm.shape[1]), device=DEVICE))  # Fix: explicit float cast avoids integer log2
    # Fix: use .item() before Python comparisons to avoid tensor bool ambiguity
    norm_entropy = (entropy / max_entropy) if max_entropy.item() > 0 else entropy
    mean_entropy = norm_entropy.mean().item()

    if mean_entropy < 0.3:
        return mean_entropy / 0.3
    elif mean_entropy > 0.7:
        return 1.0 - (mean_entropy - 0.7) / 0.3
    return 1.0
...
def test_evaluation_metrics(fret_np, raw_all, length_info,
                            load_saved_models=True, save_models=True,
...
Examples:
  ...
  %(prog)s --batch            # Process all fret pkl files
    ...
    result_dir   = root / "output" / "hmm_result"
    ckpt_hmm_dir = root / "output" / "ckpt_hmm" / "saved_hmm_models"
    ...
    fret_dir     = Path(cfg.get("hmm", {}).get("fret_dir", ""))
    pred_seg_dir = Path(cfg.get("hmm", {}).get("pred_seg_dir", ""))
