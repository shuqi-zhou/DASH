...
│  HMM_assign.py  │  scores each parameter combination → final outputs
└────────┬────────┘
         │
         ▼
   State assignments + Summary figure + Transition / emission tables
```
...
### Step 3 — HMM state assignment (`step3_HMM_assign.py`)
...
|-----------|--------|---------|
| `n_mix` | [1] | Gaussian mixture components per hidden state |
| `nstates` | [2, 3, 4, 5] | Number of hidden states |
| `global_threshold` | [0.05, 0.08, 0.1, 0.12, 0.15] | Min mean-distance to merge states globally |
| `local_threshold` | [0.05, 0.08, 0.1, 0.12, 0.15] | Min mean-distance to merge states locally per segment |
...
| File | Content |
|------|---------|
| `hmm_result/<name>/step3_HMM_final_predictions_<name>.pkl` | List of per-segment dicts (see §3.5) from the best-scoring parameter set |
| `hmm_result/<name>/step3_HMM_summary_<name>.png` | Overall and per-state FRET histograms |
...
```
┌──────────────────────────┬──────────────────────────┐
│  Overall FRET histogram  │  State-specific panels   │
│  · grey = all data       │  · grey = all data       │
│  · colours = each state  │  · colour = one state    │
│  · transparent overlays  │  · title shows occupancy │
└──────────────────────────┴──────────────────────────┘
```

The summary figure intentionally does not report Gaussian-fit parameters. It is a
compact visual check of the final state assignment and state occupancy.
...
- **Add more HMM states:** edit `NSTATES_VALUES` in `step3_HMM_assign.py`.
