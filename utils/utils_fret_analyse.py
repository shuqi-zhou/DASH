    ...
    def _handle_short_dwells(self, pred, fret, state_means, min_last_frame=3):
        ...
        for idx in range(len(all_dwell_initial)):
            ...
            if prev == next and prev is not None:
                ...
                diff_prev = abs(curr_mean - state_means[prev]) if prev is not None else float('inf')
                diff_next = abs(curr_mean - state_means[next]) if next is not None else float('inf')
                ...
                # else: both None — leave segment unchanged
