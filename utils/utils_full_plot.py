...
# Colour palette shared by Step 3 and postprocess (state index -> color)
STATE_COLORS = [
...
def plot_hmm_summary(update_predictions_all, merged_means, data_set_name,
    ...
    """
    all_fret = np.concatenate([d["fret_data"] for d in update_predictions_all]).ravel()
    all_pred = np.concatenate([d["predictions"] for d in update_predictions_all])
    states   = np.unique(all_pred)
    n_states = len(states)
    colors   = [STATE_COLORS[i % len(STATE_COLORS)] for i in range(n_states)]
    ...
    # ── Panel A: FRET Histogram (per-state coloured) ────────────────────────
    bins = np.linspace(-0.1, 1.1, 50)
    bin_width = bins[1] - bins[0]
    alphas = np.linspace(0.6, 0.25, max(n_states, 1))
    ax_hist.hist(all_fret, bins=bins, color="lightgray",
                 density=True, alpha=0.6, zorder=1)
    for i, s in enumerate(states):
        ...
        mu = np.mean(sd)
        std = np.std(sd)
        weights = np.ones(len(sd)) / (len(all_fret) * bin_width)
        ax_hist.hist(
            sd, bins=bins, weights=weights, color=colors[i],
            alpha=alphas[i], zorder=2,
            label=f"State {s}  μ={mu:.2f}  σ={std:.2f}",
        )
        ax_hist.axvline(
            mu, color=colors[i], linewidth=1.2,
